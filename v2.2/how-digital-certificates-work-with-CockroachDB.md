---
title: How digital certificates work with CockroachDB
summary: Learn about how TLS digital certificates work and how to use them with CockroachDB.
toc: true
---

This documents presents a conceptual overview of digital certificates, and then proceeds to discuss how CockroachDB uses digital certificates.
...
Thus a secure CockroachDB cluster needs to meet two requirements:

-   Allow CockroachDB to restrict access to authorized clients (or nodes acting as clients)
-   Authenticate nodes to ensure they aren’t impersonated.

To meet these requirements, CockroachDB uses [TLS 1.2](https://en.wikipedia.org/wiki/Transport_Layer_Security) digital certificates for inter-node and client-node authentication, which require a Certificate Authority (CA) as well as keys and certificates for nodes, clients, and the Admin UI. To understand how CockroachDB uses digital certificates, let's first understand what are digital certificates:
...
The TLS client certificates are used to authenticate the client connecting to a server. Because most client certificates authenticate a user instead of a device, the certificates contain usernames instead of a hostnames. This makes it difficult for public CAs to verify the client's identity and hence most public CAs will not sign a client certificate. In that case, you might need to set up your own internal CA to issue client certificates.
